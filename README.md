...
- `tc_tree` — get / children / exists / exists_batch / get_batch / get_xml / set_xml / set_xml_batch / rename / rename_batch / create / create_batch / create_rack / delete / delete_batch / import / export / focus
  ...
  - **ESI-backed rack creator (`create_rack`):** `tc_tree action:create_rack path:<parent> modules:[{type, revision?, name?}, …]` builds **functional** EtherCAT boxes from the stock Beckhoff ESI library. For each module it resolves the terminal `type` (e.g. `"EL1008"`) against `C:\Program Files (x86)\Beckhoff\TwinCAT\3.1\Config\Io\EtherCAT\*.xml` (Beckhoff vendor `Id=2`), defaulting to the **highest revision that has real PDOs** (thin "catalog stub" Device blocks in master ESI files — placeholder entries with a huge fake revision and empty-`Index` PDOs — are skipped) unless `revision` (e.g. `"#x00120000"`) is given, **generates a full `.xti`** (EtherCAT identity attributes + the complete `<Pdo>`/`<Entry>` set + box-level `<SyncMan>`/`<Fmmu>`), and **imports** it under `path` (renaming to `name` if supplied). Importing a full ESI-generated `.xti` is the **only** route that yields a non-hollow box — a bare `subType:9099` create or a PDO-less `.xti` imports as an unusable, 0-PDO ghost. The generator is anchored to TwinCAT's own output: digital was validated by diffing its `.xti` against real exported `EL1008`/`EL2008` boxes, and analog by importing the generated box and diffing TwinCAT's import-normalised export — both until the functional content matched byte-for-byte. **The box-level `<SyncMan>`/`<Fmmu>` blobs are now computed entirely from the ESI's own `<Sm>`/`<Fmmu>` declarations** by a single generic encoder pair (`Build-SyncManFromEsi` / `Build-FmmuFromEsi`) shared by digital and analog — there are **no baked per-class record templates**. See the `2026-06-21` entry in `IMPROVEMENTS.md` for the exact ESI→blob byte rule and the handful of TwinCAT-recomputed fields that are necessarily emitted as 0/derived.
    - **Scope = digital + analog-input terminals.** Supported: **EL1xxx digital-input** (ESI `GroupType` `DigIn`) and **EL2xxx digital-output** (`DigOut`), one 1-bit `BOOL` channel per PDO; and **EL3xxx analog input** (`AnaIn`, v1.1) — e.g. `EL3064`/`EL3104`/`EL3162` — which map the ESI `<TxPdo Sm="…">` SM-assigned channels to `<Pdo>` with `INT` value entries, `BIT`/`BIT2` status bits, multi-bit byte-status (`USINT`/`BYTE`/`WORD`…), and the padding gaps that make up the analog status word. Every channel's `Value` (and status leaves) is functional/linkable on import. Still-unsupported classes — **analog output** (`AnaOut`, EL4xxx — its Outputs-SM control/FMMU direction was not calibrated), IO-Link, modular devices, and any unmapped entry data type — are **detected and rejected with a clear error** rather than emitting a wrong box. Type-not-found and revision-not-found also error per-entry.
    - Modules are processed **sequentially, continue-on-error**: each is resolved → generated → written to a temp `.xti` → imported → validated (non-blank name, lands under the requested parent) → the temp file is cleaned up. Returns a compact roll-up `{ parent, count, succeeded, failed, results:[{ type, name, ok, error?, path?, productCode?, revisionNo?, pdoCount? }] }`. Accepts optional `save:true` to save the solution once after the batch (adds a `saved` boolean). Example:
      ...
      tc_tree action:create_rack
        ...
        modules:[{"type":"EL1008","name":"R01.Main.N15 (EL1008)"},{"type":"EL2008"},{"type":"EL3064"}]
      ```
      Note: terminals must be created under a **coupler** (e.g. `EK1200`/`EK1100`), not directly under the EtherCAT device.
